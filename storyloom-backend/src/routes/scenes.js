...
router.post('/generations/:generationId/accept', requireRole('creator'), asyncHandler(async (req, res) => {
...
router.patch('/:sceneId', requireRole('creator'), asyncHandler(async (req, res) => {
  const { title, tone, text } = req.body || {};
  const scene = await sceneAgent.updateScene(req.params.id, req.params.sceneId, { title, tone, text });
  res.json(scene);
}));

router.delete('/:sceneId', requireRole('creator'), asyncHandler(async (req, res) => {
...
router.post('/:sceneId/regenerate', requireRole('creator'), asyncHandler(async (req, res) => {
  const projectId = req.params.id;
  const storyContext = await projectRepo.buildStoryContext(projectId);
  if (!storyContext) return res.status(404).json({ error: 'Project not found' });
